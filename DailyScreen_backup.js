...
export default function DailyScreen({ date, onDateChange }) {
  ...
  const [pomoRemaining, setPomoRemaining] = useState(25 * 60); // seconds ? const [pomoState, setPomoState] = useState("idle"); // idle | focus | break ? const [sessionNote, setSessionNote] = useState("");
            ...
            {showCustomInput ? (
              <View style={styles.customArea}>
                <TextInput style={styles.customInput}
                  placeholder="输入科目名称" ? value={customName}
              ...
              <>
                <FlatList ? data={subjects.filter((s) => !dayData[s] || (!dayData[s].planned && (!dayData[s].sessions || dayData[s].sessions.length === 0)))}
...
const styles = StyleSheet.create({
  ...
  // Date nav ? dateNav: { flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  ...
  // Empty state ? emptyState: { alignItems: "center", paddingVertical: 60 },
  ...
  // Progress ? progressRow: { flexDirection: "row", alignItems: "center", gap: spacing.sm, marginBottom: spacing.md },
  progressNum: { ...typography.small, fontWeight: "600", minWidth: 60, textAlign: "right" },

  // Actions area ? actions: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: spacing.sm },
  ...
  // Custom input ? customArea: { padding: spacing.xl },
  customInput: {
