...
import {
  ...
  TextInput, Modal, FlatList, Platform, Vibration,
} from "react-native";
...
export default function DailyScreen({ date, onDateChange }) {
  ...
  const [quickPlanVisible, setQuickPlanVisible] = useState(false);
  ...
  // ===== 番茄钟：单 interval + 状态机 =====
  const pomoStateRef = useRef("idle");
  ...
  const tickPomodoro = useCallback(() => {
    setPomoRemaining((prev) => {
      if (prev <= 1) {
        // 当前阶段结束
        const phase = pomoStateRef.current;
        if (phase === "focus") {
          // 专注结束 → 进入休息
          Vibration.vibrate([0, 500, 200, 500]);
          pomoStateRef.current = "break";
          setPomoState("break");
          return breakLen;
        } else {
          // 休息结束 → 回到 idle
  ...
  const startPomodoro = useCallback(() => {
    stopPomodoro();
    pomoStateRef.current = "focus";
    setPomoState("focus");
    setPomoRemaining(pomoLen);
    pomoInterval.current = setInterval(tickPomodoro, 1000);
  }, [stopPomodoro, tickPomodoro]);
  ...
  const canAddSubjects = subjects.filter(
    (s) => !dayData[s] || (!dayData[s].planned && (!dayData[s].sessions || dayData[s].sessions.length === 0))
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scrollArea} showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        {/* Quick plan + Add buttons */}
        <View style={styles.bottomActions}>
          <TouchableOpacity style={styles.quickPlanBtn}
            onPress={() => setQuickPlanVisible(true)}
          >
            <Ionicons name="flash" size={18} color={colors.accent} />
            <Text style={styles.quickPlanBtnText}>快速规划</Text>
          </TouchableOpacity>
      ...
      {/* Quick plan modal */}
      <QuickPlanModal
        visible={quickPlanVisible}
        dateStr={dateStr}
        subjects={subjects}
        onApply={refresh}
        onClose={() => setQuickPlanVisible(false)}
      />
...
const styles = StyleSheet.create({
  ...
  quickPlanBtn: {
    ...
    padding: spacing.lg, borderWidth: 1, borderColor: colors.accent,
  },
  quickPlanBtnText: { ...typography.body, color: colors.accent, fontWeight: "600" },
  addBtn: {
